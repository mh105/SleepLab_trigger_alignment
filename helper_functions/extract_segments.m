...
for history_i = 1:height(history)
    ...
    if outcome == "partial_start_match"
        interval_pairs = [interval_pairs; partial_start_interval_pairs( ...
            trigger_match_result, eeg_input, edf_input, ...
            eeg_chunk_i, edf_chunk_i)]; %#ok<AGROW>
        alignment_locked = true;
    elseif any(outcome == ["match", "shared_trigger_glitch"])
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function pairs = partial_start_interval_pairs( ...
    result, eeg_input, edf_input, eeg_chunk_i, edf_chunk_i)

eeg_edges = extract_chunk_edges(eeg_input, eeg_chunk_i);
edf_edges = extract_chunk_edges(edf_input, edf_chunk_i);
eeg_start = find( ...
    eeg_edges.source_event_index == result.eeg_initial_anchor_event_index, ...
    1);
edf_start = find( ...
    edf_edges.source_event_index == result.edf_initial_anchor_event_index, ...
    1);
assert(~isempty(eeg_start) && ~isempty(edf_start), ...
    'extract_segments:InvalidPartialStartAnchor', ...
    'Partial-start anchors must be source triggers in their first chunks.')

eeg_edges = eeg_edges(eeg_start:end, :);
edf_edges = edf_edges(edf_start:end, :);
first_interval_number = ...
    result.initial_canonical_interval_number + edf_start - 1;
interval_number = (first_interval_number:height(eeg_input.canonical_cycle))';
assert(height(eeg_edges) == height(edf_edges) && ...
    height(eeg_edges) == numel(interval_number), ...
    'extract_segments:InvalidPartialStartPairing', ...
    'Partial-start interval counts do not preserve the canonical suffix.')
