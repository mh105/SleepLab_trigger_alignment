...
function testMarkedExactCountShortenedIntervalResumesSameOffset(testCase)
[eeg, edf, ~] = build_pair(complete_keep, complete_keep);
eeg_start_anchor = eeg.chunk_report.start_event_index(2) + 19;
...
eeg = attach_amplifier_interruption( ...
...
verifyFalse(testCase, result.requires_relock)
verifyEqual(testCase, result.missing_periods.reason, ...
    "amplifier_disconnect")
verifyEqual(testCase, ...
    result.missing_periods.start_anchor_event_index, eeg_start_anchor)
verifyEqual(testCase, ...
    result.missing_periods.end_anchor_event_index, eeg_end_anchor)

interruption = result.amplifier_interruptions;
verifyEqual(testCase, interruption.resolution_status, ...
    "excluded_and_relocked")
verifyTrue(testCase, interruption.exclude_from_segments)
verifyEqual(testCase, interruption.eeg_start_anchor_event_index, ...
    eeg_start_anchor)
verifyEqual(testCase, interruption.eeg_end_anchor_event_index, ...
    eeg_end_anchor)
verifyEqual(testCase, interruption.edf_start_anchor_event_index, ...
    eeg_start_anchor)
verifyEqual(testCase, interruption.edf_end_anchor_event_index, ...
    eeg_end_anchor)
verifyEqual(testCase, interruption.eeg_start_anchor_latency, ...
    double(eeg_output.event_table.latency(eeg_start_anchor)))
verifyEqual(testCase, interruption.eeg_end_anchor_latency, ...
    double(eeg_output.event_table.latency(eeg_end_anchor)))
verifyEqual(testCase, interruption.edf_start_anchor_latency, ...
    double(edf_output.event_table.latency(eeg_start_anchor)))
verifyEqual(testCase, interruption.edf_end_anchor_latency, ...
    double(edf_output.event_table.latency(eeg_end_anchor)))
end

function testMarkedWithinToleranceIntervalIsRetainedAndAudited(testCase)
...
verifyEqual(testCase, ...
    result.amplifier_interruptions.edf_start_anchor_event_index, ...
    eeg_start_anchor)
verifyEqual(testCase, ...
    result.amplifier_interruptions.edf_end_anchor_event_index, ...
    eeg_end_anchor)
end

function testMarkedMissingTriggersUsesExplicitDisconnectBracket(testCase)
[eeg, edf, ~] = build_pair(short_loss_keep, complete_keep);
eeg_start_anchor = eeg.chunk_report.start_event_index(2) + 10;
eeg_end_anchor = eeg_start_anchor + 1;
eeg = attach_amplifier_interruption( ...
    eeg, eeg_start_anchor, eeg_end_anchor, "paired");
...
verifyTrue(testCase, ...
    result.amplifier_interruptions.exclude_from_segments)
verifyEqual(testCase, ...
    result.amplifier_interruptions.edf_start_anchor_event_index, 61)
verifyEqual(testCase, ...
    result.amplifier_interruptions.edf_end_anchor_event_index, 64)
end

function testMarkedMultiIntervalBracketForcesExclusion(testCase)
[eeg, edf, ~] = build_pair(complete_keep, complete_keep);
eeg_start_anchor = eeg.chunk_report.start_event_index(2) + 19;
...
function testDiscrepancyOutsideMarkedIntervalRemainsUnsupported(testCase)
[eeg, edf, ~] = build_pair(complete_keep, complete_keep);
chunk_start = eeg.chunk_report.start_event_index(2);
eeg_start_anchor = chunk_start + 5;
eeg_end_anchor = eeg_start_anchor + 1;
outside_discrepancy_end = chunk_start + 25;
eeg.event_table.latency(outside_discrepancy_end:end) = ...
    eeg.event_table.latency(outside_discrepancy_end:end) - eeg.Fs;
eeg = attach_amplifier_interruption( ...
    eeg, eeg_start_anchor, eeg_end_anchor, "paired");
...
verifyEqual(testCase, result.missing_periods.reason, ...
    "manual_cycle_count_review")
verifyEqual(testCase, ...
    result.amplifier_interruptions.resolution_status, "manual_review")
verifyFalse(testCase, ...
    result.amplifier_interruptions.exclude_from_segments)
end

function testWholeCycleMarkedBracketStopsForManualReview(testCase)
[eeg, edf, ~] = build_pair(complete_keep, complete_keep);
eeg_start_anchor = eeg.chunk_report.start_event_index(2) + 19;
eeg_end_anchor = eeg_start_anchor + 50;
eeg = attach_amplifier_interruption( ...
    eeg, eeg_start_anchor, eeg_end_anchor, "paired");
...
function testCrossBoundaryBracketRelocksWithoutCycleAmbiguity(testCase)
[eeg, edf, ~] = build_pair(complete_keep, complete_keep);
eeg_start_anchor = eeg.chunk_report.end_event_index(2) - 1;
eeg_end_anchor = eeg.chunk_report.start_event_index(3) + 1;
eeg = attach_amplifier_interruption( ...
    eeg, eeg_start_anchor, eeg_end_anchor, "paired");
...
verifyEqual(testCase, ...
    ...
    eeg_end_anchor)
verifyEqual(testCase, ...
    result.comparison_history.outcome(2:3), ...
    ["eeg_data_loss"; "eeg_data_loss"])
end

function testTerminalMissingReconnectUsesLastTrustedAnchor(testCase)
[eeg, edf, ~] = build_pair( ...
    partial_final_keep(30), partial_final_keep(30));
eeg_start_anchor = eeg.chunk_report.start_event_index(end) + 10;
eeg = attach_amplifier_interruption( ...
...
function testLeadingInterruptionStartsAtFirstConfirmedPostgapTrigger(testCase)
[eeg, edf, ~] = build_pair(complete_keep, complete_keep);
eeg_end_anchor = 3;
eeg.event_table.latency(1:eeg_end_anchor - 1) = ...
    eeg.event_table.latency(1:eeg_end_anchor - 1) + eeg.Fs;
eeg = attach_amplifier_interruption( ...
    eeg, NaN, eeg_end_anchor, "paired");

[result, eeg_output, ~] = run_match(eeg, edf);

verifyEqual(testCase, result.eeg_initial_anchor_event_index, ...
    eeg_end_anchor)
verifyEqual(testCase, result.edf_initial_anchor_event_index, ...
    eeg_end_anchor)
...
function testPairedTerminalInterruptionKeepsConfirmedSuffix(testCase)
[eeg, edf, ~] = build_pair( ...
    partial_final_keep(30), partial_final_keep(30));
eeg_start_anchor = eeg.chunk_report.start_event_index(end) + 10;
eeg_end_anchor = eeg_start_anchor + 2;
eeg = attach_amplifier_interruption( ...
    eeg, eeg_start_anchor, eeg_end_anchor, "paired");
...
function system = attach_amplifier_interruption( ...
    system, eeg_start_anchor, eeg_end_anchor, pairing_status)

disconnect_raw_event_index = height(system.event_table) + 1;
if isfinite(eeg_start_anchor)
    disconnect_latency = double( ...
        system.event_table.latency(eeg_start_anchor));
    eeg_start_anchor_latency = disconnect_latency;
else
    disconnect_latency = double(system.event_table.latency(1)) - 1;
    eeg_start_anchor_latency = NaN;
end
disconnect_event_text = "9001, Amplifier disconnected";

if pairing_status == "paired"
    reconnect_raw_event_index = disconnect_raw_event_index + 1;
    reconnect_latency = double( ...
        system.event_table.latency(eeg_end_anchor));
    reconnect_event_text = "9002, Amplifier reconnected";
    eeg_end_anchor_latency = reconnect_latency;
else
    assert(pairing_status == "missing_reconnect", ...
        'Unsupported synthetic amplifier interruption status.')
    reconnect_raw_event_index = NaN;
    reconnect_latency = NaN;
    reconnect_event_text = "";
    eeg_end_anchor = NaN;
    eeg_end_anchor_latency = NaN;
end
