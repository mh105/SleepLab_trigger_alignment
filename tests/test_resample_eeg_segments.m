...
[edf_input, aligned_eeg_channel_names] = make_edf_input( ...
...
function testReportsProgressPerSegmentAndChannel(testCase)
edf_Fs = 256;
edf_trigger_Fs = 128;
eeg_sample_span = [1000; 1500];
edf_trigger_sample_span = [256; 383];
eeg_segment_data = { ...
    zeros(2, eeg_sample_span(1) + 1, 'single'); ...
    zeros(2, eeg_sample_span(2) + 1, 'single')}; %#ok<NASGU>
matched_segments = make_segment_table( ...
    eeg_sample_span, edf_trigger_sample_span); %#ok<NASGU>
[edf_input, aligned_eeg_channel_names] = make_edf_input(edf_Fs, edf_trigger_Fs, 2); %#ok<ASGLU>

display_text = evalc( ...
    ['resample_eeg_segments(eeg_segment_data, matched_segments, ' ...
    'edf_input, aligned_eeg_channel_names);']);

expected_channel_messages = [ ...
    "Segment 1/2, channel 1/2 (Fp1): high-pass filtering"; ...
    "Segment 1/2, channel 2/2 (Fp2): high-pass filtering"; ...
    "Segment 2/2, channel 1/2 (Fp1): high-pass filtering"; ...
    "Segment 2/2, channel 2/2 (Fp2): high-pass filtering"];
for message_i = 1:numel(expected_channel_messages)
    verifyTrue(testCase, contains( ...
        string(display_text), expected_channel_messages(message_i)))
end
verifyTrue(testCase, contains(display_text, ...
    'Segment 1/2: exact-grid resampling 100% complete'))
verifyTrue(testCase, contains(display_text, ...
    'Segment 2/2: exact-grid resampling 100% complete'))
end
...
[resampled, sanity_figure_handles] = resample_eeg_segments( ...
    eeg_segment_data, matched_segments, edf_input, ...
    aligned_eeg_channel_names, true);
...
function [edf_input, aligned_eeg_channel_names] = make_edf_input( ...
    edf_Fs, edf_trigger_Fs, n_channels)
