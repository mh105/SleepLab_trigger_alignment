...
edf_input.DC_trace = zeros(10, 1);
aligned_eeg_channel_names = { ...
    'Fp1', 'Fp2', 'F3', 'F4', 'C3', ...
    'C4', 'O1', 'O2', 'M1', 'M2', 'VEOGL'};
expected_edf_eeg_total_sample_count = 20;
...
expected = zeros( ...
    11, expected_edf_eeg_total_sample_count, 'single');
...
verifySize(testCase, final_eeg_data, ...
    [11 expected_edf_eeg_total_sample_count])
...
expected_mask = false(1, expected_edf_eeg_total_sample_count);
...
verifySize(testCase, matched_edf_sample_mask, ...
    [1 expected_edf_eeg_total_sample_count])
